...
$.fn.ProvinceCity = function(){
	...
	_self.append("<select style='margin-left:20px;'></select>");
	_self.append("<select style='margin-left:20px;'></select>");*/
	//分别获取3个下拉框
...
/*	//默认省级下拉
	...
	$.each( GP , function(index,data){
		if(data==$sel1.attr('zhi')){
			$sel1.append("<option selected='selected' value='"+data+"'>"+data+"</option>");
		}else{
...
/*		if(index1==0){	//当选择的为 “请选择” 时
			...
			$.each( GT[index1] , function(index,data){
				if($sel2.attr('zhi')==data){
					$sel2.append("<option selected='selected' value='"+data+"'>"+data+"</option>");
				}else{
					$sel2.append("<option value='"+data+"'>"+data+"</option>");
				}
			});
			$.each(GC[$sel1.get(0).selectedIndex][$sel2.get(0).selectedIndex], function(index,data){
				if($sel3.attr('zhi')==data){
					$sel3.append("<option selected='selected' value='"+data+"'>"+data+"</option>");
				}else{
	...
	$sel2.change(function(){
		...
		$.each( GC[index1][index2] , function(index,data){
				$sel3.append("<option value='"+data+"'>"+data+"</option>");
		});
	});
	return _self;
